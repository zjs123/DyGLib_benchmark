...
if __name__ == "__main__":
    ...
    for run in range(args.num_runs):
        ...
        for epoch in range(args.num_epochs):
            print(model[0].embedding_module.linear_layer.weight[0][:10])
            model.train()
...
if __name__ == "__main__":
    ...
    for run in range(args.num_runs):
        ...
        for epoch in range(args.num_epochs):
            print(model[0].node_raw_features[0][:10])
            model[0].train()
            model[1].train()
            if args.model_name in ['DyRep', 'TGAT', 'TGN', 'CAWN', 'TCL', 'GraphMixer', 'DyGFormer']:
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                with torch.no_grad():
                    if args.model_name in ['TGAT', 'CAWN', 'TCL']:
                        ...
                        raise ValueError(f"Wrong value for model_name {args.model_name}!")
                # get predicted probabilities, shape (batch_size, )
                predicts = model[1](x_1=batch_src_node_embeddings, x_2 = batch_dst_node_embeddings, rel_embs = model[0].edge_raw_features).sigmoid()
                pred_labels = torch.max(predicts, dim=1)[1]
                labels = torch.from_numpy(batch_labels).long().to(predicts.device)
